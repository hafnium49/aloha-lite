...
def write_csv_output(rulespecs: List[Dict], args, repo_id: str, output_path: str):
    """Write the rule specifications to a CSV file."""
    ...
    # Create header
    header = [
        'stage', 'episode', 'dataset', 
        'left_j1', 'left_j2', 'left_j3', 'left_j4', 'left_j5', 'left_j6',
        'right_j1', 'right_j2', 'right_j3', 'right_j4', 'right_j5', 'right_j6'
    ]
    
    # Add optional cartesian columns if present
    if any('left_cart' in spec for spec in rulespecs):
        header.extend(['left_x', 'left_y', 'left_z', 'left_rx', 'left_ry', 'left_rz'])
    if any('right_cart' in spec for spec in rulespecs):
        header.extend(['right_x', 'right_y', 'right_z', 'right_rx', 'right_ry', 'right_rz'])
    
    # Add gripper columns if present
    if any('left_grip' in spec for spec in rulespecs):
        header.append('left_gripper')
    if any('right_grip' in spec for spec in rulespecs):
        header.append('right_gripper')
    ...
    for i, spec in enumerate(rulespecs):
        ...
        left_joints = spec.get('left_j', [0] * 6)
        row.extend(left_joints[:6])  # Ensure exactly 6 joints
        
        # Right arm joints (6 DOF)
        right_joints = spec.get('right_j', [0] * 6)
        if right_joints and not args.single_arm:
            row.extend(right_joints[:6])
        else:
            row.extend([0] * 6)  # Empty values for single arm
        
        # Optional cartesian data
        if any('left_cart' in spec for spec in rulespecs):
            left_cart = spec.get('left_cart', [0] * 6)
            row.extend(left_cart[:6])
            
        if any('right_cart' in spec for spec in rulespecs):
            right_cart = spec.get('right_cart', [0] * 6)
            row.extend(right_cart[:6] if right_cart and not args.single_arm else [0] * 6)
        
        # Optional gripper data
        if any('left_grip' in spec for spec in rulespecs):
            row.append(spec.get('left_grip', 0))
        if any('right_grip' in spec for spec in rulespecs):
            row.append(spec.get('right_grip', 0))
    ...
    print(f"✅  Wrote {len(rulespecs)} rules → {output_path}")
...
def main(argv=None):
    ...
    if output_path.suffix.lower() == '.csv':
        # Generate CSV output
        write_csv_output(rulespecs, args, repo_id, str(output_path))
    else:
        # Generate Python rules output (original behavior)
        ...
        output_path.write_text(whole_script)
        print(f"✅  Wrote {len(rulespecs)} rules → {output_path}")
